...
exports.listPicture = function(req, res) {
  ...
  // NOTE: This field returns the image of the category.
  res.jsonp(category.icon);
...
exports.changePicture = function (req, res) {
  ...
  var message = null;
  var upload = multer(config.uploads.categoriesUpload).single('newPicture');
  ...
  upload(req, res, function (uploadError) {
    if(uploadError) {
      ...
      category.icon = config.uploads.categoriesUpload.dest + req.file.filename;
      category.save(function (saveError) {
        if (saveError) {
          return res.status(400).send({
